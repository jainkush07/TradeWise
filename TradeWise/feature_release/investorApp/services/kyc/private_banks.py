...
from investorApp.models import (investorPersonalDetails, investorBankDetails, investorDMATDetails,
                                UserTransferFacility, CompanyDetails, ChannelPartnerDetails, UserSocialDetails)
from investorApp.services.kyc.config import PrivateBanksKycConfig, StepName, KycFeConfig
from investorApp.services.kyc.screen_configs.pb_config import (COMPANY_DETAIL_SECTION,
...
class PrivateBankKyc(KycBase):
    role = UserRoleTypes.PRIVATE_BANKS.value
    _instance = None

    def __init__(self, user):
        self.user = user
        self.steps = PrivateBanksKycConfig
        self.common_kyc = CommonKycConfig(self.user, self.role)
        self.investor_obj = investorPersonalDetails.fetch_obj(user.id)
        super(PrivateBankKyc, self).__init__(user)
    ...
    def _fetch_company_detail_conf(self):
        ...
        sector_data = list(obj.company_sector.all().values('id', 'name'))
        data = obj.__dict__
        sectors = []
        sector_data_map = {}
        for sector in sector_data:
            sectors.append(sector['id'])
            sector_data_map[sector['id']] = sector['name']
