...
def logged_in_user_redirect(request, callingFunction=None):
	...
	if request.user.is_authenticated:
		...
		try:
			...
			elif not userInst.userProfile_otp_authentication_authV2.phone_verified and not userInst.first_name == 'employeeProfile':
...
def password_reset_view(request, slug=None):
	...
	if page_flag == 'reset':
		if request.GET.get('token') or request.POST.get('token'):
			...
			user = User.objects.get(pk=user_id)
			otpInst, otpCreated = otp_authentication.objects.get_or_create(userProfile=user)
			if otpCreated and user.first_name == 'employeeProfile':
				request.session['send_on'] = sent_otp_on = 'Email'
				otpInst.email = user.email
			otp = generate_otp_view()
			otpInst.otp = otp
			otpInst.save()
			errorsCount = 0
